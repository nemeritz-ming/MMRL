        ...
        if len(self.lob_record.keys()) != 0:
            for set_price in self.lob_record.keys():
                if self.lob_record[set_price] > 0 and set_price >= self.b1:
                    ...
                    Trading_Records.append(f"{self.a1}, buy , {abs(self.lob_record[set_price])}, {self.shares_held}, {self.balance}")
                    ...
                    self.shares_held -= -self.lob_record[set_price]
                    Trading_Records.append(f"{self.b1}, sell , {abs(self.lob_record[set_price])}, {self.shares_held}, {self.balance}")
        ...
        for action in actions:
            ...
            if action_type == 0:
                ...
                if amount > self.b1:
                    ...
                    self.shares_held += 1
                    # print(f"{self.b1}, buy , 1 , {self.shares_held}, {self.balance}")
                    Trading_Records.append(f"{self.prev_time},{self.b1}, buy , 1 , {self.shares_held}, {self.balance}")
                ...
                if amount < self.a1:
                    ...
                    self.shares_held -= 1
                    # print(f"{self.a1}, sell , 1 , {self.shares_held}, {self.balance}")
                    Trading_Records.append(f"{self.prev_time},{self.a1}, sell , 1 , {self.shares_held}, {self.balance}")
                ...
                # print(f"{mid_price}, {tx} ,{abs(self.shares_held)}, 0, {self.balance}")
...
if __name__ == '__main__':
    for day in ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10",
        ...
        for i in range(num):
            ...
            while not done:
                ...
                if time > time_e:
                    ...
                    a = buy_ans[buy_ans['col2'] >= buy_ans[buy_ans['col2'] > 0]['col2'].quantile(0.8)]['price'].values
                    b = sell_ans[sell_ans['col2'] >= sell_ans[sell_ans['col2'] > 0]['col2'].quantile(0.8)][
                        'price'].values
                    target = np.intersect1d(a, b)
                    for sell_buy_price in target:
                        if abs(market_price[-1] - sell_buy_price) <= delta and abs(my_inv) <= 3:
        ...
        print('end at:', day)
        # with open("result_with_fee{0}.txt".format(day), "w") as output:
        #     output.write(str(profit))
        # with open("trades_with_fee{0}.txt".format(day), "w") as output:
        #     output.write(str(Trading_Records))
        Trading_Records = []
