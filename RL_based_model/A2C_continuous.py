...
class Agent(object):
    ...
    def choose_action(self, observation):
        # mean of action prob, and var of action,
        mu, sigma = self.actor.forward(observation)  # .to(self.actor.device)
        sigma = T.exp(sigma)  # make var > 0
        action_probs = T.distributions.Normal(mu, sigma)  # create normal distribution N(mu, sigma)
        probs = action_probs.sample(sample_shape=T.Size([self.n_outputs]))  # sample one result from this distribution
        self.log_probs = action_probs.log_prob(sum(probs)).to(self.actor.device)  # calculate  log prob
        action = T.abs(T.tanh(probs))  # get action
