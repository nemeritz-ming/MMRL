...
class ReplayBuffer():
    ...
    def sample_buffer(self, batch_size):
        # first_idx = np.random.choice(self.mem_cntr - batch_size, 1, replace=False)
        batch = np.array(np.arange(self.mem_cntr-batch_size, self.mem_cntr,1))
...
class Agent():
    ...
    def reset_buffer(self):
        self.memory = ReplayBuffer(self.mem_size, self.input_dim)
    ...
    def learn(self):
        ...
        states_ = states_.to(T.float32)
        states = states.to(T.float32)
