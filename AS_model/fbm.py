...
# if __name__ == '__main__':
    # def quantize(S, k=60):
    #     N = len(S)
    #     minChange = (max(S) - min(S)) / k
    #     Q = np.zeros((N, 1))
    #     Q[0] = S[0]
    #     for t in range(1, N):
    #         if abs(S[t] - S[t - 1]) > minChange:
    #             Q[t] = S[t]
    #         else:
    #             Q[t] = Q[t - 1]
    #     return Q
    # f0 = FBM(n=1024, hurst=0.7, length=1, method='daviesharte')
    # f1 = FBM(n=1024, hurst=0.4, length=1, method='daviesharte')
    # f2 = FBM(n=1024, hurst=0.6, length=1, method='daviesharte')
    # t = list(range(1025))
    # print(len(f0.fbm()))
    #
    # Q = quantize(f0.fbm())
    # plt.plot(t, Q, label = 'quantized S')
    # plt.plot(t, f0.fbm(), label = 'S')
    # plt.legend(loc=0)
    # plt.show()

    # plt.plot(t, f0.fbm(), label='h=0.7')
    # plt.plot(t, f1.fbm(), label='h=0.4')
    # plt.plot(t, f2.fbm(), label='h=0.6')
    # plt.legend(loc=0)
    # plt.grid(True)
    # plt.show()
